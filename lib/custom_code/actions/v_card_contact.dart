...
Future<String?> vCardContact(
    DocumentReference? userRef, DocumentReference? profileRef) async {
  var vCard = VCard();

  String? userId = userRef?.id;
  String? contactID = profileRef?.id;

  DocumentSnapshot? proDoc = await getProfileDoc(profileRef!.id, userId!);
  ...
  //vCard.photo.embedFromFile(proDoc?.get("contact_image"));
  ...
  final newvCardRef =
      storageRef.child("users/" + userId! + "/" + contactID! + "vCard.vcf");
...
Future<DocumentSnapshot?> getProfileDoc(String profileID, String userID) async {
